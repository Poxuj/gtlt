				...
				{
					{
						...
						p.Insert("`4ALREADY ON?! `O:");
						p.CreatePacket(peer);
					}
					{
						gamepacket_t p;
						p.Insert("OnConsoleMessage");
						p.Insert("`oThis account was already online, kicking it off so you can log on. (If you were just playing before, this is nothing to worry about");
						p.CreatePacket(peer);
					}
				...
				if (cch.find("action|dialog_return") == 0)
				{
					...
					if (isRegisterDialog) {
						...
						if (regState == 1) {
							...
							packet::consolemessage(peer, "`4Account creation has failed, `obecause it already exists!``");
						}
						else if (regState == -2) {
							packet::consolemessage(peer, "`4Account creation has failed, `obecause the name is too short!``");
						}
						else if (regState == -3) {
							packet::consolemessage(peer, "`4Passwords mismatch!``");
						}
						else if (regState == -4) {
							packet::consolemessage(peer, "`4Account creation has failed, `obecause email address is invalid!``");
						}
						else if (regState == -5) {
							packet::consolemessage(peer, "`4Account creation has failed, `obecause Discord ID is invalid!``");
						}
					}
#endif
				}
