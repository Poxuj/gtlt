			...
			case 2:
			{
				...
				if (cch.find("action|growid") == 0)
				{
							...
							if (regState == 1)
							{
								...
								bool success = true;
								SaveShopsItem(6336, 1, peer, success);
								Player::OnSetBux(peer, 30000, 1);
								string lastworld = username;
								packet::dialog(peer, "\nadd_label|big|`$Welcome `@" + lastworld + "`$!|left|\nadd_spacer|small|\nadd_label_with_icon|small|`5Your account information :|left|32|\nadd_smalltext|`oYour username: `2" + username + "|left|\nadd_smalltext|`oYour password: `2" + password + "|left|\nadd_smalltext|`oYour email: `2" + email + "|left|\nadd_spacer|small|\nadd_smalltext|`oYou recieved `230,000 `ogems!|left|\nadd_spacer|small|\nadd_quick_exit|\nend_dialog||Thankyou|");
				...
				if(pStr.substr(0, 17) == "action|enter_game" && !((PlayerInfo*)(event.peer->data))->isIn)
				{
					...
					packet::consolemessage(peer, "Server made by Growtopia Noobs, edited by King and Modiz");
					PlayerInventory inventory;
					for (int i = 0; i < 200; i++)
					{
						InventoryItem it;
